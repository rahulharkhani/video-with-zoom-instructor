...
class Zoom_Video_Conferencing_Admin_PostType {
	...
	public function __construct() {
		...
		if(isset($_GET['post']) && isset($_GET['action']) && $_GET['post'] != "" && $_GET['action'] == "edit") {
			$author_id = get_post_field( 'post_author', $_GET['post'] );
			$user_id = $author_id;
			$this->api_key    = get_user_meta( $user_id, 'zoom_api_key', true );
			$this->api_secret = get_user_meta( $user_id, 'zoom_api_secret', true );
		}else{
