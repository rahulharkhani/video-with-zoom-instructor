...
class Zoom_Video_Conferencing_Admin_Views {
	...
	public function zoom_video_conference_menus() {
		if(is_admin()) {
			...
			$zoom_api_key    = get_user_meta( $user_id, 'zoom_api_key', true );
			$zoom_api_secret = get_user_meta( $user_id, 'zoom_api_secret', true );
			//if ( get_option( 'zoom_api_key' ) && get_option( 'zoom_api_secret' ) && video_conferencing_zoom_api_get_user_transients() ) {
			if ( $zoom_api_key && $zoom_api_secret && video_conferencing_zoom_api_get_user_transients() ) {	
		...
		/* add zoom_menu_access for admin and teacher role */
		add_submenu_page( 'edit.php?post_type=zoom-meetings', __( 'Settings', 'video-conferencing-with-zoom-api' ), __( 'Settings', 'video-conferencing-with-zoom-api' ), 'zoom_menu_access', 'zoom-video-conferencing-settings', array(
			$this,
        ...
        <div class="wrap">
			...
			if ( 'api-settings' === $active_tab ) {
				if ( isset( $_POST['save_zoom_settings'] ) ) {
					...
					update_user_meta( $user_id, 'zoom_api_key', $zoom_api_key );
					update_user_meta( $user_id, 'zoom_api_secret', $zoom_api_secret );
					update_user_meta( $user_id, 'zoom_vanity_url', $vanity_url );
					update_user_meta( $user_id, 'zoom_api_donot_delete_on_zoom', $delete_zoom_meeting );
					update_user_meta( $user_id, 'zoom_past_join_links', $join_links );
					update_user_meta( $user_id, 'zoom_show_author', $zoom_author_show );
					update_user_meta( $user_id, 'zoom_started_meeting_text', $started_mtg );
					update_user_meta( $user_id, 'zoom_going_tostart_meeting_text', $going_to_start );
					update_user_meta( $user_id, 'zoom_ended_meeting_text', $ended_mtg );
					update_user_meta( $user_id, 'zoom_api_date_time_format', $locale_format );
					update_user_meta( $user_id, 'zoom_api_custom_date_time_format', $custom_date_time_format );
					update_user_meta( $user_id, 'zoom_api_full_month_format', $full_month_format );
					update_user_meta( $user_id, 'zoom_api_twenty_fourhour_format', $twentyfour_format );
					update_user_meta( $user_id, 'zoom_api_embed_pwd_join_link', $embed_pwd_in_join_link );
					update_user_meta( $user_id, 'zoom_api_hide_shortcode_join_links', $hide_join_links_non_loggedin_users );
					update_user_meta( $user_id, 'zoom_api_hide_in_jvb', $hide_email_jvb );
					update_user_meta( $user_id, 'vczapi_disable_invite', $vczapi_disable_invite );
					update_user_meta( $user_id, 'zoom_api_disable_jvb', $disable_join_via_browser );
					update_user_meta( $user_id, 'zoom_api_default_lang_jvb', $join_via_browser_default_lang );
					update_user_meta( $user_id, 'zoom_api_disable_auto_meeting_pwd', $disable_auto_pwd_generation );
				...
				$zoom_api_key                = get_user_meta( $user_id, 'zoom_api_key', true );
				$zoom_api_secret             = get_user_meta( $user_id, 'zoom_api_secret', true );
				$zoom_vanity_url             = get_user_meta( $user_id, 'zoom_vanity_url', true );
				$past_join_links             = get_user_meta( $user_id, 'zoom_past_join_links', true );
				$zoom_author_show            = get_user_meta( $user_id, 'zoom_show_author', true );
				$zoom_started                = get_user_meta( $user_id, 'zoom_started_meeting_text', true );
				$zoom_going_to_start         = get_user_meta( $user_id, 'zoom_going_tostart_meeting_text', true );
				$zoom_ended                  = get_user_meta( $user_id, 'zoom_ended_meeting_text', true );
				$locale_format               = get_user_meta( $user_id, 'zoom_api_date_time_format', true );
				$custom_date_time_format     = get_user_meta( $user_id, 'zoom_api_custom_date_time_format', true );
				$twentyfour_format           = get_user_meta( $user_id, 'zoom_api_twenty_fourhour_format', true );
				$full_month_format           = get_user_meta( $user_id, 'zoom_api_full_month_format', true );
				$embed_password_join_link    = get_user_meta( $user_id, 'zoom_api_embed_pwd_join_link', true );
				$embed_password_join_link    = get_user_meta( $user_id, 'zoom_api_embed_pwd_join_link', true );
				$hide_join_link_nloggedusers = get_user_meta( $user_id, 'zoom_api_hide_shortcode_join_links', true );
				$hide_email_jvb              = get_user_meta( $user_id, 'zoom_api_hide_in_jvb', true );
				$vczapi_disable_invite       = get_user_meta( $user_id, 'vczapi_disable_invite', true );
				$disable_jvb                 = get_user_meta( $user_id, 'zoom_api_disable_jvb', true );
				$default_jvb_lang            = get_user_meta( $user_id, 'zoom_api_default_lang_jvb', true );
				$disable_auto_pwd_generation = get_user_meta( $user_id, 'zoom_api_disable_auto_meeting_pwd', true );
				$donot_delete_zoom           = get_user_meta( $user_id, 'zoom_api_donot_delete_on_zoom', true );
