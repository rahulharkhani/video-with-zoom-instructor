...
if ( ! class_exists( 'Zoom_Video_Conferencing_Api' ) ) {
	...
	class Zoom_Video_Conferencing_Api {
		...
		public function __construct( $zoom_api_key = '', $zoom_api_secret = '' ) {
			...
			$this->zoom_api_secret = get_user_meta( $user_id, 'zoom_api_secret', true );
		}
		...
		protected function sendRequest( $calledFunction, $data, $request = "GET" ) {
			...
			$args        = array(
				'headers' => array(
					'Authorization' => 'Bearer ' . $this->generateJWTKey( $calledFunction ),
		...
		/**
		 * Return the current logged in user id if the current post type is zoom-meetings then it returns the post_author id
		 * @author Prajesh Vekariya
		 */
		static function get_user_id( $zoom_api_endpoint = '', $zoom_meeting_id = 0 ) {
			
			$current_user = wp_get_current_user();
			$user_id = $current_user->ID;

			/* update post */
			if( isset( $_REQUEST['post_type'] ) && $_REQUEST['post_type'] == 'zoom-meetings' 
				&& isset( $_REQUEST['post_author'] ) && intval( $_REQUEST['post_author'] ) > 0 ) {

				$user_id = intval( $_REQUEST['post_author'] );

			/*edit post*/
			} else if ( isset( $_GET['post'] ) && isset( $_GET['action'] ) && $_GET['action'] == 'edit' && intval( $_GET['post'] ) > 0 ) {

				$user_id = get_post_field( 'post_author', intval( $_GET['post'] /*post => post_id*/ ) );

			} else if( substr( $zoom_api_endpoint, 0, 9 ) === 'meetings/' ) {
				$zoom_meeting_id = str_replace( 'meetings/', '', $zoom_api_endpoint );
				
				$user_id = get_post_field( 'post_author', get_post_id_from_zoom_meeting_id( $zoom_meeting_id ) );
				
			} else if( substr( $zoom_api_endpoint, 0, 6 ) === 'users/' ) {

				$user_id = str_replace( 'users/', '', $zoom_api_endpoint );
				
			} else if ( $zoom_meeting_id > 0 ) {
				$user_id = get_post_field( 'post_author', get_post_id_from_zoom_meeting_id( $zoom_meeting_id ) );
			}
		...
		private function generateJWTKey( $zoom_api_endpoint = '' ) {
			//$key    = $this->zoom_api_key;
			//$secret = $this->zoom_api_secret;

			$user_id = self::get_user_id( $zoom_api_endpoint );

			$key    = get_user_meta( $user_id, 'zoom_api_key', true );
			$secret = get_user_meta( $user_id, 'zoom_api_secret', true );
